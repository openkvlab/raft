...
func TestProtoMemorySizes(t *testing.T) {
	...
	var e Entry
	assert.Equal(t, if64Bit(80, 40), unsafe.Sizeof(e), "Entry size check")
	...
	var m Message
	assert.Equal(t, if64Bit(192, 96), unsafe.Sizeof(m), "Message size check")

	var hs HardState
	assert.Equal(t, if64Bit(56, 28), unsafe.Sizeof(hs), "HardState size check")
	...
	assert.Equal(t, if64Bit(48, 24), unsafe.Sizeof(ccs), "ConfChangeSingle size check")
