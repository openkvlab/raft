...
func TestMsgAppFlowControlMoveForward(t *testing.T) {
	...
	for tt := 2; tt < r.trk.MaxInflight; tt++ {
		...
		r.Step(pb.Message{From: 2, To: 1, Type: pb.MessageType_MsgAppResp, Index: uint64(tt)})
		r.readMessages()
		...
		// ensure 2
		for i := 0; i < tt; i++ {
			r.Step(pb.Message{From: 2, To: 1, Type: pb.MessageType_MsgAppResp, Index: uint64(i)})
			require.True(t, pr2.IsPaused())
		}
	}
}
