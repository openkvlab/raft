...
func TestMsgAppFlowControlMoveForward(t *testing.T) {
	...
	for tt := 2; tt < r.trk.MaxInflight; tt++ {
		...
		r.Step(&pb.Message{From: new(uint64(2)), To: new(uint64(1)), Type: new(pb.MessageType_MsgAppResp), Index: new(uint64(tt))})
		r.readMessages()
		...
		// ensure 2
		for i := 0; i < tt; i++ {
			r.Step(&pb.Message{From: new(uint64(2)), To: new(uint64(1)), Type: new(pb.MessageType_MsgAppResp), Index: new(uint64(i))})
			require.True(t, pr2.IsPaused())
		}
	}
}
